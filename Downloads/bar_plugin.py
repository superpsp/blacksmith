...
register_command_handler(handler_bar_control, 'бар*', ['фан','все'], 10, 'Добавить/удалить напитки в бар, без параметров покажет содержимое бара. Название напитка должно быть написано в винительном (для одной тары) или родительном (для нескольких тар) падеже, т.к. будет использоваться в виде "/me достал из бара <название_напитка> для <ник>". max кол-во пользовательских напитков - 1000', 'бар* [+/адд/-/дел]', ['бар* + бочонок пива','бар* - 4','бар*'])