...
register_command_handler(show_statconfs, 'посещаемость', ['все','админ'], 20, 'Просмотр статистики посещаемости конференций в которых сидит бот за последние пять дней, без параметров выдаст статистику по текущей конференции, с параметром "лист" выдаст статистику по всем конференциям (выполнение с параметрами доступно только для суперадминов)\nBy WitcherGeralt\nhttp://witcher-team.ucoz.ru/', 'посещаемость [конференция/list/лист]', ['посещаемость','посещаемость witcher@conference.jabber.ru','посещаемость list'])
