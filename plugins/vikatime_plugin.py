...
def handler_vika_time(type, source, body):
#	try:
#		body = re_search(read_url('http://www.zln.ru.sixxs.org/time/', 'Mozilla/5.0'), '<div id="servertime" style="margin-top:40px; margin-bottom:30px; height:44px; padding:6px; width:148px; border:2px dotted #990000; font-size:36px; font-weight:bold;">', '</div>')
		...
		repl += u'\nГод: %s' % (god)
		repl += u'\n-----'
		repl += u'\nВремя у Вики: %s' % ( str(int(time.strftime('%H'))+5)+':'+minut+sek)
		repl += u'\n-----'
...
register_command_handler(handler_vika_time, 'настенныечасы', ['инфо','все'], 10,'Показывает точное московское время с поправкой для Томска, взятое с серверов времени из интернета.','настенныечасы', ['настенныечасы'])
