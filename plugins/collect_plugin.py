...
#			message = random.choice([u'чистка...', u'Работа по антиупарыванию конфы в разгаре!', 'вычищаю конференцию', 'отсылаю пустые мессаги (не правда ли дебильная работа?)'])
			message = random.choice([u'чистка...', u'Зачистка кишлака в разгаре!', u'Вычищаю конференцию', u'Нифига не буду - плати бабло!', u'Ведро-тряпку давай, да!'])
#			message = u'Зачистка кишлака в разгаре!'
		...
		if type != 'private':
			...
			change_bot_status(source[1], message, status)
		reply(type, source, u'Кишлак зачищен, командир!')
...
def handler_test(type, source, body):
	testfr = [u'Чё нада?', u'А! Чё!? Где? фууу, на_х так пугать!', u'Провален, твой IQ = 90!', u'Пассед', u'Ща те такой тест устрою!', u'- Тест на Дебила выключен, ты опоздал', u'Во ***! Нах разбудил! Мне снилась прекрасная Isida...', u'Сейчас, сейчас! Протестим твою репу на удароустойчивость!']
...
register_command_handler(handler_clean, 'чисть', ['все','разное'], 10, 'Чистит конфу', 'чисть', ['чисть'])
...
register_command_handler(handler_admin_message, 'мессага', ['овнер','все'], 30, 'Отправляет мессагу от имени бота определённому jid`у.', 'мессага [jid] [мессага]', ['мессага chuvak@jabber.org здарова чувак!'])
